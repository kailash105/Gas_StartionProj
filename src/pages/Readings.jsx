...
const Readings = () => {
    const { readings, setReadings } = useAppContext();
    ...
    const handleDelete = (e, date) => {
        ...
        if (confirm('Are you sure you want to delete this reading?')) {
            setReadings(readings.filter(r => r.date !== date));
        }
    };
                            ...
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    <h3 className="text-lg font-semibold text-slate-800">
                                        {new Date(reading.date).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                                    </h3>
                                    <p className="text-slate-500 text-sm">Total Usage: {reading.totalUsage.toLocaleString()} L</p>
                                </div>
